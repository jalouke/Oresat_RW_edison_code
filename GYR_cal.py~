...
def floating_array_filter(in_window,in_temp):
	in_window.append(in_temp)
	in_window.pop(0)
	out_n = manual_filt_low(b,a,in_temp,out_temp)
...
while timer<15:
        a=time.time()
	GYRx, out_x_window, in_x_window = floating_array_filter(in_x_window,(readGYRx()- GYRx_bias))
	GYRy, out_y_window, in_y_window = floating_array_filter(in_y_window,(readGYRy()- GYRy_bias))
	GYRz, out_z_window, in_z_window = floating_array_filter(in_z_window,(readGYRz()- GYRz_bias))
	bias_totx += GYRx
	bias_toty += GYRy
	bias_totz += GYRz
	count+=1
